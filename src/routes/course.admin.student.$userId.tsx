...
import { adminGetStudentAccess, adminSetVideoAccess } from "@/server/admin";
...
export const Route = createFileRoute("/course/admin/student/$userId")({
...
function StudentAccessPage() {
  ...
  return (
    <div className="space-y-4 pb-24">
      <button onClick={() => navigate({ to: "/course/admin/students" })} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
      ...
      {filteredTree.length === 0 ? (
        <Card className="p-10 text-center text-sm text-muted-foreground">
          {tree.length === 0 ? (
            <>কোনো কোর্স তৈরি হয়নি। প্রথমে <Link to="/course/admin/courses" className="font-semibold text-primary hover:underline">কোর্স যোগ করুন</Link>।</>
          ) : "মিল পাওয়া যায়নি"}
        </Card>
      ) : (
