...
export function AppHeader() {
  ...
  const isAdminArea = loc.pathname.startsWith("/course/admin");
  ...
  return (
    <header className="sticky top-0 z-40 border-b-2 border-border/60 bg-background/85 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
        <Link to="/course" className="flex items-center gap-2 transition-transform hover:scale-[1.03]" aria-label="Learniby course portal home">
          <img src={logo} alt="Learniby learning portal logo" className="h-9 w-auto sm:h-10" />
        </Link>

        <nav className="flex items-center gap-1">
          {role === "admin" && (
            <Link to={isAdminArea ? "/course" : "/course/admin"}>
              <Button
          ...
          <Link to="/course/profile" aria-label="Open your profile">
